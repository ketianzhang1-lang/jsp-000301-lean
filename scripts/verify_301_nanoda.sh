...
evidence=evidence/strict-301/nanoda
mkdir -p "$evidence"
work=$(mktemp -d)
...
lake env "$work/exporter/.lake/build/bin/lean4export" JSP000301 -- "${targets[@]}" > "$evidence/export.ndjson"
...
report = {
    ...
    'module': 'JSP000301', 'targets': (out / 'targets.txt').read_text().splitlines(),
