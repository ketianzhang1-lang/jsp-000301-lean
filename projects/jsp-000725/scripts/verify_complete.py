#!/usr/bin/env python3
"""Verify the complete pinned admissible-set proof and the integration theorem."""
...
for n in ['JSP000725','JSP000725Bridge','JSP000725Complete','AuditComplete']:sources[n]=Path(n+'.lean')
order=[]
def visit(n):
    ...
    order.append(n)
visit('AuditComplete')
if len(order)!=46 or len(sources)!=46:raise SystemExit('Incomplete source closure')
...
if len(expected)!=28:raise SystemExit('Unexpected audit target count')
...
for n in ['ErdosProblems','JSP000725','JSP000725Bridge','JSP000725Complete']:
    run(['lake','env','leanchecker','--verbose',n],'kernel-'+n+'.log')
negative=Path('NegativeComplete.lean');negative.write_text('import JSP000725Complete\nexample : (1 : Nat) = 0 := by decide\n')
...
'dependency_count':len(manifest['packages']),'kernel_prefixes':['ErdosProblems','JSP000725','JSP000725Bridge','JSP000725Complete'],
