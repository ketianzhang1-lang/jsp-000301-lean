#!/usr/bin/env python3
"""Compile and audit the complete pinned JSP-000907 development."""
...
run(['sha256sum','--check','ORIGINAL_NOTE_SHA256'],'original-note.log')
...
for n in ['JSP000907Construction', 'JSP000907OddRim', 'JSP000907Witness', 'JSP000907Complete', 'AuditComplete']:sources[n]=Path(n+'.lean')
...
visit('AuditComplete')
if set(order)!=set(sources) or len(order)!=43:raise SystemExit('Unexpected unused source manifest entry')
...
workers=int(os.environ.get('JSP907_BUILD_JOBS','2'))
if workers not in (1,2,3):raise SystemExit('JSP907_BUILD_JOBS must be 1, 2 or 3')
    ...
    while pending or running:
        for n in order:
            ...
            if resume.get(n)==fingerprints[n] and out.exists() and log.exists() and not re.search(r'\berror:',log.read_text()):
                ...
                continue
            future=pool.submit(run,['lake','env','lean','-DwarningAsError=true','-j1','-M10000','-o',str(out),'./'+str(sources[n])],'build-'+n+'.log')
            running[future]=n
        if not running:
            if failed:break
            if pending:continue
...
if len(expected)!=35 or len(set(expected))!=35:raise SystemExit('Unexpected audit targets')
...
prefixes=['ErdosProblems', 'JSP000907Construction', 'JSP000907OddRim', 'JSP000907Witness', 'JSP000907Complete']
for n in prefixes:
    run(['lake','env','leanchecker','--verbose',n],'kernel-'+n+'.log')
negative=Path('NegativeComplete.lean');negative.write_text('import JSP000907Complete\nexample : (1 : Nat) = 0 := by decide\n')
