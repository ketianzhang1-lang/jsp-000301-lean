#!/usr/bin/env python3
"""Verify the complete pinned counting proof and the integration theorem."""
...
for n in ['JSP000728','JSP000728Bridge','JSP000728Complete','AuditComplete']:sources[n]=Path(n+'.lean')
order=[]
def visit(n):
    ...
    order.append(n)
visit('AuditComplete')
if len(order)!=47 or len(sources)!=47:raise SystemExit('Incomplete source closure')
...
if len(expected)!=27:raise SystemExit('Unexpected audit target count')
...
for n in ['ErdosProblems','JSP000728','JSP000728Bridge','JSP000728Complete']:
    run(['lake','env','leanchecker','--verbose',n],'kernel-'+n+'.log')
negative=Path('NegativeComplete.lean');negative.write_text('import JSP000728Complete\nexample : (1 : Nat) = 0 := by decide\n')
...
'dependency_count':len(manifest['packages']),'kernel_prefixes':['ErdosProblems','JSP000728','JSP000728Bridge','JSP000728Complete'],
