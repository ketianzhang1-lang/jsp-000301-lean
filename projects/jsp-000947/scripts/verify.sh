...
lake env leanchecker Main 2>&1 | tee evidence/leanchecker.log
...
for name in names:
    ...
    assert m is not None or ('\"'+name+'\" does not depend on any axioms') in log or ("'"+name+"' does not depend on any axioms") in log,name
