...
evidence=evidence/generated/nanoda
mkdir -p "$evidence"
work=$(mktemp -d)
...
if len(targets) != 11 or len(set(targets)) != 11:
...
lake env "$work/exporter/.lake/build/bin/lean4export" JSP001021 -- "${targets[@]}" > "$evidence/export.ndjson"
...
report = {
    ...
    'module': 'JSP001021', 'targets': (out / 'targets.txt').read_text().splitlines(),
