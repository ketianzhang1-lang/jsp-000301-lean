#!/usr/bin/env python3
"""Verify the complete pinned graph-subdivision proof and the integration theorem."""
...
for n in ['Certificate', 'GraphCore', 'Profile', 'Subdivision', 'Colouring', 'Catlin', 'Sharp', 'JSP000585Bridge', 'JSP000585Complete', 'AuditComplete']:sources[n]=Path(n+'.lean')
order=[]
def visit(n):
    ...
    order.append(n)
visit('AuditComplete')
if len(order)!=66 or len(sources)!=66:raise SystemExit('Incomplete source closure')
...
if len(expected)!=61:raise SystemExit('Unexpected audit target count')
...
for n in ['ErdosProblems', 'Certificate', 'GraphCore', 'Profile', 'Subdivision', 'Colouring', 'Catlin', 'Sharp', 'JSP000585Bridge', 'JSP000585Complete']:
    run(['lake','env','leanchecker','--verbose',n],'kernel-'+n+'.log')
negative=Path('NegativeComplete.lean');negative.write_text('import JSP000585Complete\nexample : (1 : Nat) = 0 := by decide\n')
...
'dependency_count':len(manifest['packages']),'kernel_prefixes':['ErdosProblems', 'Certificate', 'GraphCore', 'Profile', 'Subdivision', 'Colouring', 'Catlin', 'Sharp', 'JSP000585Bridge', 'JSP000585Complete'],
