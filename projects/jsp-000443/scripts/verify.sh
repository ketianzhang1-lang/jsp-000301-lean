...
lake env leanchecker JSP000443 2>&1 | tee evidence/kernel-main.log
lake env leanchecker PolarityFamily 2>&1 | tee evidence/kernel-family.log
...
print('Sixteen target axiom audits and all nine dependency revision checks passed.')
PY
printf 'import PolarityFamily\nexample : (1 : Nat) = 0 := by decide\n' > Negative.lean
...
python3 scripts/generate_polarity.py | tee evidence/crosscheck.json
