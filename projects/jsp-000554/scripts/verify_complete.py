#!/usr/bin/env python3
"""Verify the complete pinned rough-prime-gap proof and the integration theorem."""
...
def visit(n):
    ...
    for d in re.findall(r'^(?:public )?import\s+(\S+)',sources[n].read_text(),re.M):
        ...
        elif d.startswith(('ErdosProblems.', 'PrimeNumberTheoremAnd.', 'Util.', 'UnitFractions.')):
            raise SystemExit('Unpinned upstream import: '+d)
    order.append(n)
visit('AuditComplete')
if len(order)!=54 or len(sources)!=54:raise SystemExit('Incomplete source closure')
...
if len(expected)!=37:raise SystemExit('Unexpected audit target count')
...
for n in ['ErdosProblems', 'PrimeNumberTheoremAnd', 'Util', 'UnitFractions', 'JSP000554', 'JSP000554Complete']:
...
'dependency_count':len(manifest['packages']),'kernel_prefixes':['ErdosProblems', 'PrimeNumberTheoremAnd', 'Util', 'UnitFractions', 'JSP000554', 'JSP000554Complete'],
