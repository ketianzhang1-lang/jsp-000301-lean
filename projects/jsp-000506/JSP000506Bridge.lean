...
@[simp] theorem fromGraph_toGraph {n : ℕ} (s : Finset (Edge n)) :
  ...
  ext e
  rw [mem_fromGraph, toGraph_adj_iff s e.property]

@[simp] theorem toGraph_fromGraph {n : ℕ} (G : SimpleGraph (Fin n)) :
    toGraph (fromGraph G) = G := by
  ext u v
  rcases lt_trichotomy u v with h | rfl | h
  · rw [toGraph_adj_iff _ h, mem_fromGraph]
  · simp
  · rw [SimpleGraph.adj_comm, toGraph_adj_iff _ h, mem_fromGraph, SimpleGraph.adj_comm]
...
theorem coProper_iff_all_pairs {n k : ℕ} (s : Finset (Edge n)) :
  ...
  · rintro ⟨c,t,ht⟩
    refine ⟨c,t,?_⟩
    intro e heq
    rw [← toGraph_adj_iff s e.property]
    exact ht _ _ (ne_of_lt e.property) heq
...
theorem coProper_iff_coColorable {n k : ℕ} (s : Finset (Edge n)) :
  ...
  constructor
  · rintro ⟨c,t,ht⟩
    refine ⟨{color := c, kind := fun i => if t i then .clique else .independent,
      valid := ?_}⟩
    intro i
    cases hi : t i with
    | false =>
