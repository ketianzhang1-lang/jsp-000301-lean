#!/usr/bin/env python3
"""Compile and audit the complete pinned JSP-000506 development."""
...
if 'version 4.31.0,' not in version:raise SystemExit('Wrong compiler: '+version)
(EVIDENCE/'lean-version.txt').write_text(version)
run(['sha256sum','--check','ORIGINAL_PROOF_SHA256'],'original-proof.log')
embedded=re.findall(r'^BEGIN SOURCE MODULE: (\S+)',Path('Erdos625SelfContained.lean').read_text(),re.M)
if len(embedded)!=480 or len(set(embedded))!=480:raise SystemExit('Unexpected upstream closure')
...
sources={x['path'][:-5].replace('/','.'):Path(x['path']) for x in json.loads(Path('UPSTREAM.json').read_text()) if x['path'].endswith('.lean')}
for n in ['JSP000506', 'JSP000506Bridge', 'JSP000506Complete', 'AuditComplete']:sources[n]=Path(n+'.lean')
...
visit('AuditComplete')
if set(order)!=set(sources) or len(order)!=5:raise SystemExit('Unexpected unused source manifest entry')
...
workers=int(os.environ.get('JSP506_BUILD_JOBS','2'))
if workers not in (1,2,3):raise SystemExit('JSP506_BUILD_JOBS must be 1, 2 or 3')
    ...
    while pending or running:
        for n in order:
            ...
            if resume.get(n)==fingerprints[n] and out.exists() and log.exists() and not re.search(r'\berror:',log.read_text()):
                ...
                continue
            future=pool.submit(run,['lake','env','lean','-DwarningAsError=true','-j1','-M12000','-o',str(out),'./'+str(sources[n])],'build-'+n+'.log')
            running[future]=n
        if not running:
            if failed:break
            if pending:continue
...
if len(expected)!=49 or len(set(expected))!=49:raise SystemExit('Unexpected audit targets')
local_names=set()
for filename in ['JSP000506.lean','JSP000506Bridge.lean','JSP000506Complete.lean']:
    local_names.update('JSP000506.'+n for n in re.findall(r'^(?:@\[[^\n]*\]\s*)?(?:theorem|lemma)\s+(\w+)',Path(filename).read_text(),re.M))
if len(local_names)!=45 or not local_names<=set(expected):raise SystemExit('Missing local theorem audit')
if json.loads(Path('AUDIT_TARGETS.json').read_text())!=expected:raise SystemExit('Exporter/audit target mismatch')
...
prefixes=['Erdos625SelfContained', 'JSP000506', 'JSP000506Bridge', 'JSP000506Complete']
for n in prefixes:
    run(['lake','env','leanchecker','--verbose',n],'kernel-'+n+'.log')
negative=Path('NegativeComplete.lean');negative.write_text('import JSP000506Complete\nexample : (1 : Nat) = 0 := by decide\n')
...
'modules_compiled':order,'upstream_embedded_source_modules':480,'axiom_reports':len(expected),'allowed_axioms':sorted(allowed),
