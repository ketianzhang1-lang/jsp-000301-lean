...
lake env leanchecker JSP000636 2>&1 | tee evidence/leanchecker-upper.log
lake env leanchecker Lower 2>&1 | tee evidence/leanchecker-lower.log
lake env lean Audit.lean 2>&1 | tee evidence/axioms.log
python3 - <<'PY'
import json, re, subprocess
from pathlib import Path
source = '\n'.join(Path(f).read_text() for f in ['JSP000636.lean', 'Lower.lean'])
...
print('All 38 theorem axiom audits and dependency pins passed.')
PY
