...
evidence=evidence/supplement/nanoda
mkdir -p "$evidence"
work=$(mktemp -d)
...
lake env "$work/exporter/.lake/build/bin/lean4export" JSP000897Stability -- "${targets[@]}" > "$evidence/export.ndjson"
...
report = {
    ...
    'module': 'JSP000897Stability', 'targets': (out / 'targets.txt').read_text().splitlines(),
