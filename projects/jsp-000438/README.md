...
3. **Sharpness at every even order.** We construct explicit cyclic colorings and prove that the star on `2*k+2` vertices has Ramsey number exactly `4*k+2` for every natural `k`, including zero.
...
We request formalization credit for our Ramsey integration, sharpness proofs, compatibility port and verification work. The mathematical implications and constructions are classical.
