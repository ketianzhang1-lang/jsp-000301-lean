...
mapfile -t targets < <(sed -n 's/^#print axioms //p' Audit.lean)
lake env "$work/exporter/.lake/build/bin/lean4export" JSP000254 -- "${targets[@]}" \
...
config = {
    ...
    "pp_declars": ["JSP000254.cambieSet", "JSP000254.extremalCount",
                   "Erdos302.NoUnitFractionTriple", "Erdos302.IsMaxNoTripleCard"] +
                  [line.split()[-1] for line in Path("Audit.lean").read_text().splitlines()
