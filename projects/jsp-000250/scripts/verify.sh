...
lake env leanchecker JSP000250 2>&1 | tee evidence/leanchecker.log
...
src=Path('JSP000250.lean').read_text()
assert not re.search(r'\b(sorry|admit|axiom|native_decide|unsafe)\b',src)
print('All eight axiom audits, source checks and actual dependency revisions passed.')
PY
printf 'import JSP000250\nexample : (1 : Nat) = 0 := by decide\n' > Negative.lean
