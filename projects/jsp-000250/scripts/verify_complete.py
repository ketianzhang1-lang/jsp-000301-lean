#!/usr/bin/env python3
"""Compile and audit the complete pinned JSP-000250 development."""
...
for n in ['JSP000250','JSP000250Complete','AuditComplete']:sources[n]=Path(n+'.lean')
...
def visit(n):
    ...
    for d in re.findall(r'^(?:public )?import\s+(\S+)',sources[n].read_text(),re.M):
        ...
        elif d.startswith(('ErdosProblems.','UnitFractions.','PrimeNumberTheoremAnd.')):
            raise SystemExit('Unpinned upstream import: '+d)
    visiting.remove(n);order.append(n)
visit('AuditComplete')
if set(order)!=set(sources):raise SystemExit('Unexpected unused source manifest entry')
...
workers=int(os.environ.get('JSP250_BUILD_JOBS','2'))
if workers not in (1,2,3):raise SystemExit('JSP250_BUILD_JOBS must be 1, 2 or 3')
...
if len(expected)!=21 or len(set(expected))!=21:raise SystemExit('Unexpected audit targets')
...
prefixes=['PrimeNumberTheoremAnd','UnitFractions','ErdosProblems','JSP000250','JSP000250Complete']
for n in prefixes:
    run(['lake','env','leanchecker','--verbose',n],'kernel-'+n+'.log')
negative=Path('NegativeComplete.lean');negative.write_text('import JSP000250Complete\nexample : (1 : Nat) = 0 := by decide\n')
