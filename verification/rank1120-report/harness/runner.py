...
def run(name,args,cwd=project,limit=1800):
 ...
 (out/(name+'.json')).write_text(json.dumps({'argv':args,'cwd':str(cwd),'exit_code':code,'seconds':time.time()-start,'log':str(out/(name+'.log')),'log_sha256':hashlib.sha256((out/(name+'.log')).read_bytes()).hexdigest()}))
...
if stage=='prepare':
 ...
 for d in [cfg['evidence_dir']]+cfg.get('additional_evidence_dirs',[]):
  evidence_path=project/d
  assert not pathlib.PurePosixPath(d).is_absolute() and '..' not in pathlib.PurePosixPath(d).parts and d not in ('','.','/'), 'Unsafe evidence output directory'
  assert evidence_path.resolve().is_relative_to(project.resolve()), 'Evidence output escapes project'
 ...
 before=hashes(sorted(set(names+extras)));before.update(frozen);(out/'source-before.json').write_text(json.dumps(before,indent=2))
 ...
 (out/'export-stderr.log').write_bytes(r.stderr);(out/'export-command.json').write_text(json.dumps({'argv':args,'cwd':str(project),'exit_code':r.returncode,'seconds':time.time()-start,'log':str(out/'export-stderr.log'),'log_sha256':hashlib.sha256(r.stderr).hexdigest()}))
