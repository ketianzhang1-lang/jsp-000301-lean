...
theorem uniform_ratio_literal (ε : ℝ) (hε : 0 < ε) :
  ...
  simpa only [extremal_formula] using
    Filter.eventually_atTop.mp (JSP000465.uniform_ratio_small ε hε)
...
theorem adaptive_ratio_literal (σ : ℕ → FiniteGraph)
    ...
    Tendsto (fun n : ℕ => (literalExtremal proposedFamily n : ℝ) /
      (SimpleGraph.extremalNumber n (σ n).graph : ℝ)) atTop (nhds 0) := by
  simpa only [extremal_formula] using JSP000465.adaptive_ratio_tendsto σ hσ
      ...
      ¬ ∃ (σ : ℕ → FiniteGraph) (K : ℝ),
        ...
        (∀ᶠ n : ℕ in atTop,
          (SimpleGraph.extremalNumber n (σ n).graph : ℝ) ≤
            K * (literalExtremal F n : ℝ)) := by
  simpa only [extremal_formula] using JSP000465.adaptive_compactness_counterexample
