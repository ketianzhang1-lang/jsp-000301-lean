...
def run(name,args,cwd=project,limit=1200):
    ...
    if code:
        print((out/(name+'.log')).read_text(errors='replace')[-12000:],flush=True)
...
if stage=='prepare':
    ...
    run('bootstrap',['python3','scripts/bootstrap.py'],limit=1800)
    run('cache',['lake','exe','cache','get']+(['Mathlib'] if key=='250' else []),limit=1800)
    if key=='250':run('prepare-mathlib',['lake','build','Mathlib'],limit=1800)
    ...
    inputs=tracked+([e['path'] for e in json.loads((project/'UPSTREAM.json').read_text())] if key=='250' else ['Erdos1114.lean'])
    before={p:hashlib.sha256((project/p).read_bytes()).hexdigest() for p in inputs if (project/p).is_file()}
    ...
    run('historical-verifier',['bash','scripts/verify.sh'],limit=5400)
    shutil.copytree(project/config['evidence_dir'],out/'fresh-project-evidence',dirs_exist_ok=True)
    if any(p.startswith(config['evidence_dir']+'/') for p in tracked):
        run('restore-historical-evidence',['git','restore','--',config['evidence_dir']])
    ...
    run('official-audit',['python3','/harness/audit.py','run',str(target),'--out','/out/audit','--lake','/opt/elan/toolchains/leanprover--lean4---v4.34.0/bin/lake','--timeout','1200'],limit=5400)
    run('bridge-kernel',['lake','env','leanchecker',config['bridge_module']],limit=1800)
    ...
    snap=out/'checked-source';snap.mkdir(exist_ok=True)
    for name in before:
        if name.endswith(('.lean','.json','.md')) and not name.startswith('evidence'):
            dest=snap/name;dest.parent.mkdir(parents=True,exist_ok=True);shutil.copyfile(project/name,dest)
