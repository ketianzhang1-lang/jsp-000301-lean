#!/usr/bin/env python3
"""Disclosed manual build equivalence for two Lean 4.31 audit modules.

This is an auditor-owned adapter, not Lake. Only the two exact build argv
...
MODULES = {
    "JSP000506Selection": "JSP000506Selection.lean",
    "JSP000506.VerificationSelection": "JSP000506/VerificationSelection.lean",
}
OUT = Path("/out")
...
def main():
    ...
    record = {
        ...
        "reason": "The selected Lake configuration does not register the selection module. Compile the exact source using the original pinned Lean invocation.",
    }
    code = 127
    with os.fdopen(fd, "wb") as log:
        try:
            if (cwd / "lean-toolchain").read_text().strip() != "leanprover/lean4:v4.31.0":
                raise RuntimeError("Manual adapter is restricted to the pinned Lean 4.31 project")
